...
add_action('widgets_init', 'boal_information');

function boal_information()
{
...
class information_widget extends WP_Widget
{
    ...
    {
        /* Widget settings. */
        $widget_ops = array('classname' => 'information_widget', 'description' => esc_html__('Easy add image for sidebar.', 'boal'));

        /* Widget control settings. */
        $control_ops = array('width' => 250, 'height' => 350, 'id_base' => 'information_widget');

        /* Create the widget. */
        parent::__construct('information_widget', esc_html__('+NA: Image', 'boal'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        ?>
        <div class="boal-image-content">
            <?php if ($image) {
                ?>
                <a class="clearfix" href="<?php echo esc_url($link); ?>">
                    <img class="boal-image" src="<?php echo esc_url($image)?>" alt="img" />
                </a>
            <?php } ?>
        </div>
        <?php
    ...
    function form($instance)
    {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('link')); ?>"><?php echo esc_html_e('Url:', 'boal'); ?></label>
